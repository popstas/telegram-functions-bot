...
import { replaceToolPlaceholders, __clearToolCache } from "../../src/helpers/placeholders.ts";
import { ConfigChatType, ThreadStateType, ChatToolType } from "../../src/types.ts";
...
describe("replaceToolPlaceholders", () => {
  ...
  it("replaces tool placeholder with tool result", async () => {
    ...
    const res = await replaceToolPlaceholders(text, chatTools, chatConfig, thread);
  ...
  it("caches tool result", async () => {
    const text = 'Status: {tool:echo({"a":1})}';
    const res1 = await replaceToolPlaceholders(text, chatTools, chatConfig, thread, 3600);
    const res2 = await replaceToolPlaceholders(text, chatTools, chatConfig, thread, 3600);
